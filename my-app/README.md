...
1.  **`develop` ブランチ (開発環境 / Development)**
    *   **役割**: 日々の開発と統合テストを行う場所です。
    *   **DB接続先**: **Supabase Dev** (実験用データ)
    *   **デプロイ**: マージされると `preview` チャンネルに自動配信されます。
    *   **開発フロー**: 新機能は `feature/*` ブランチで開発し、`develop` にプルリクエストを送ります。

2.  **`main` ブランチ (本番環境 / Production)**
    *   **役割**: ユーザーに公開される安定版です。
    *   **DB接続先**: **Supabase Prod** (本番データ)
    *   **デプロイ**: マージされると `production` チャンネルに自動配信 (OTA) されます。
    *   **リリース**: Gitタグ (`v*`) をプッシュすると、ストア提出用ビルドが作成されます。
